...
)
{
    ...
    for (unsigned int i = 0; i < vertexIndices.size(); i++) {
        model.push_back(out_vertices[i].x);
        model.push_back(out_vertices[i].y);
        model.push_back(out_vertices[i].z);
        model.push_back(out_normals[i].x);
        model.push_back(out_normals[i].y);
        model.push_back(out_normals[i].z);
        model.push_back(out_uvs[i].x);
        model.push_back(out_uvs[i].y);
    }
